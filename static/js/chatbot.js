...
function getPublicIP() {
    ...
    $.ajax({
        ...
        // url: "http://localhost:5000/api/processInput", // Assuming your backend API endpoint is "/api/ipify"
        dataType: "json",
...
//	const serverURL = "http://127.0.0.1:5000/api/processInput";
    ...
    $.ajax({
        ...
        responseText2=response.responseText2;
        suggestions = response.suggestions;
        suggestions2 = response.suggestions2;
        ...
        const responseHtml2 = createResponseTextHtml2(responseText2);
        document.getElementById("chatBotMainContainer").innerHTML += responseHtml2;
        ...
        //     const suggestionHtml2 = createSuggestionHtml2(suggestions2);
        //     document.getElementById("chatBotMainContainer").innerHTML += suggestionHtml2;
        // }
        if (suggestions.length > 0) {
            const suggestionHtml = createSuggestionHtml(suggestions);
            const chatOptionsContainer = document.getElementById("chatOptions");
            if (chatOptionsContainer) {
                chatOptionsContainer.innerHTML = suggestionHtml;
            } else {
                document.getElementById("chatBotMainContainer").innerHTML += suggestionHtml;
            }
        }
        
        if (suggestions2 !== undefined && suggestions2 !== null) {
            const suggestionHtml2 = createSuggestionHtml2(suggestions2);
            const chatOptions2Container = document.getElementById("chatOptions2");
            if (chatOptions2Container) {
                chatOptions2Container.innerHTML = suggestionHtml2;
            } else {
                document.getElementById("chatBotMainContainer").innerHTML += suggestionHtml2;
            }
        }
...
function createResponseTextHtml2(responseText2) {

    let responseHtml2 =
        '<div id="chatBotMain2" class="row"> ' +
        '<div class="chat-message message-came quick-reply from-them bot-message  new first"> ' +
        '<span class="from-icon" src="images/mf-logo.jpg"></span> ' +
        '<div>' + '<p>'+responseText2 +'</p>'+ '</div> ' +
        '</div> ' +
        '<div class="clearfix"> </div>' +
        '<div class="timestamp-bot">' + timeFormatter() + '</div> ' +
        '</div>';

    return responseHtml2;
...
// function createSuggestionHtml2(suggestions2) {

//     let suggestionHtml2 = '<div id="chatOptions2" class="">';

//     for (let suggestion of suggestions2) {
//         // console.log('suggestion = ', suggestion)
//         suggestionHtml2 += '<button class="option " onclick="javascript:callToAction(' +
//             "'" + suggestion.suggestionInput + "'" +
//             ')" style="">' +
//             suggestion.suggestionText + '</button>';
//     }
...
// Assuming you have suggestions and suggestions2 available in the script
